...
def calculate_vital_signs(red_data, ir_data):
    # (Logika kalkulasi sama persis seperti sebelumnya)
    r = np.array(red_data)
    ir = np.array(ir_data)
    r_mean = np.mean(r)
    ir_mean = np.mean(ir)
    ...
    for i in range(1, len(ir_ac)):
        if ir_ac[i-1] < 0 and ir_ac[i] >= 0:
            if (ir_ac[i] - ir_ac[i-1]) > 5: peaks += 1
...
# MAIN PROGRAM
m = max30102.MAX30102()
print(f"[INFO] Sensor MAX30102 (QoS Mode). Target: {OVS_IP}:{OVS_PORT}")
...
try:
    while True:
        ...
        if ir_avg > 50000:
            bpm_val, spo2_val = calculate_vital_signs(red_buf, ir_buf)
            if not (40 < bpm_val < 180): # Filter noise
                bpm_val = 0
                spo2_val = 0.0
        
        # FORMAT PAYLOAD DENGAN TIMESTAMP
        payload = {
            "HeartRate": bpm_val,
            "SpO2": spo2_val,
