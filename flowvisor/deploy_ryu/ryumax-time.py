...
# ==============================================================================
# UPDATED: Port disesuaikan ke 5000 agar match dengan influx-api-all.py
INFLUX_ENDPOINT = "http://192.168.111.129:5000/sensor-max"
...
class SensorPacketHandler(app_manager.RyuApp):
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in body:
            ...
            # DATA RX/TX (TETAP DIPERTAHANKAN)
            # Kita TIDAK menyuntikkan 'ryu_timestamp' di sini agar API tidak menghitung latency untuk paket ini.
            # Timestamp bawaan hanyalah waktu pencatatan statistik biasa.
            traffic_data = {
                "type": "traffic_stats", 
                "dpid": dpid,
                "port": port_num,
    ...
    # ==========================================================================
    # BAGIAN PACKET IN (Sensor Data JSON - PORT 4)
    # ==========================================================================
        ...
        if udp_pkt and udp_pkt.dst_port == 9999:
            try:
                ...
                if json_start != -1:
                    ...
                    # [QoS INJECTION]
                    # Hanya data SENSOR yang diberi ryu_timestamp untuk perhitungan latency di API
                    current_time = time.time()
                    json_data['ryu_timestamp'] = current_time
                    
                    self.logger.info("✅ PACKET-IN: Injecting Timestamp %s", current_time)
                    self.logger.info("📥 Processing JSON from port 4: %s", json_data)
    ...
    def _post_request(self, data):
        try:
            ...
            # LOG ERROR: Jangan diam saja kalau gagal kirim
