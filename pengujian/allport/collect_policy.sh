...
# mapping port number -> interface name (OVS)
IF_P1="dht11"
IF_P2="camera"
IF_P4="max"
...
echo "[START] $(date -u +"%Y-%m-%dT%H:%M:%SZ") bridge=$BRIDGE duration=$DURATION interval=$INTERVAL linkcap=${LINK_CAP_Mbps}Mbps" > "$LOG"

echo "timestamp,\
rx_mbps_p1,tx_mbps_p1,rx_pps_p1,tx_pps_p1,avg_rx_pkt_bytes_p1,util_rx_pct_p1,drop_p1,delay_ms_p1,last_payload_bytes_p1,priority_tag_p1,policing_rate_kbps_p1,policing_burst_kbps_p1,\
...
rx_mbps_p4,tx_mbps_p4,rx_pps_p4,tx_pps_p4,avg_rx_pkt_bytes_p4,util_rx_pct_p4,drop_p4,delay_ms_p4,last_payload_bytes_p4,priority_tag_p4,policing_rate_kbps_p4,policing_burst_kbps_p4" > "$OUT"
...
get_delay_and_payload () {
  ...
  # delay_log format v2:
  # timestamp,device_id,src_ip,seq,payload_bytes,oneway_delay_ms
...
# ✅ ambil external_ids:priority + policing
get_port_qos_cfg () {
  local ifname="$1"
  ...
  # priority tag (external_ids)
  priority=$(sudo ovs-vsctl get interface "$ifname" external_ids 2>/dev/null \
    | sed -n 's/.*priority="\([0-9]\+\)".*/\1/p' | head -n 1)
  priority=${priority:-0}

  # policing rate/burst (kbps)
  ...
  # kalau ovs-vsctl return [] atau kosong
  if [[ "$rate" == "[]" || -z "$rate" ]]; then rate="0"; fi
  if [[ "$burst" == "[]" || -z "$burst" ]]; then burst="0"; fi
...
while [ "$(date +%s)" -lt "$END" ]; do
  ...
  # ✅ QoS cfg per port
  read prio_p1 rate_p1 burst_p1 <<< $(get_port_qos_cfg "$IF_P1")
  read prio_p2 rate_p2 burst_p2 <<< $(get_port_qos_cfg "$IF_P2")
  read prio_p4 rate_p4 burst_p4 <<< $(get_port_qos_cfg "$IF_P4")

  echo "$TS,\
$p1_rx_mbps,$p1_tx_mbps,$p1_rx_pps,$p1_tx_pps,$p1_avg_rx_pkt,$p1_util,$p1_drop_delta,$delay_p1,$last_bytes_p1,$prio_p1,$rate_p1,$burst_p1,\
$p2_rx_mbps,$p2_tx_mbps,$p2_rx_pps,$p2_tx_pps,$p2_avg_rx_pkt,$p2_util,$p2_drop_delta,$delay_p2,$last_bytes_p2,$prio_p2,$rate_p2,$burst_p2,\
$p4_rx_mbps,$p4_tx_mbps,$p4_rx_pps,$p4_tx_pps,$p4_avg_rx_pkt,$p4_util,$p4_drop_delta,$delay_p4,$last_bytes_p4,$prio_p4,$rate_p4,$burst_p4" >> "$OUT"
  ...
  rows=$(($(wc -l < "$OUT") - 1))
  echo "[PROGRESS] ts=$TS rows=$rows dht11_delay=$delay_p1 cam_delay=$delay_p2 max_delay=$delay_p4 policing(p1/p2/p4)=${rate_p1}/${rate_p2}/${rate_p4}kbps" | tee -a "$LOG"
