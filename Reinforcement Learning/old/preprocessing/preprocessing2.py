        ...
        for action_id, weights in ACTIONS.items():
            ...
            for p in ['p1', 'p2', 'p4']:
                ...
                if current_sim_delay > SLA_DELAY_MS[p]:
                    # Jika melanggar SLA, penaltinya harus terasa
                    reward_p -= 80 if p == 'p4' else 40 
                else:
                    # JIKA SUKSES (di bawah SLA), beri bonus tambahan
                    # Ini agar AI punya motivasi mengejar delay sekecil mungkin
                    reward_p += 20 

                # Penalti untuk P4 yang diabaikan (Safety Guard) dinaikkan
